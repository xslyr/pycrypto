...
from app.commons.utils import BrokerUtils, DataSources
...
SMATarget = Enum('SMATarget', ['open', 'close','high','low','base_asset_volume','quote_asset_volume','number_of_trades','taker_buy_base_asset_volume','taker_buy_quote_asset_volume'] )
...
class TechnicalAnalysis(np.ndarray):
    ...
    def __lt__(self, other): return self[-1] < self.__get_value(other)
    def __le__(self, other): return self[-1] <= self.__get_value(other)
    def __gt__(self, other): return self[-1] > self.__get_value(other)
    def __ge__(self, other): return self[-1] >= self.__get_value(other)
    def __eq__(self, other): return self[-1] == self.__get_value(other)
    def __ne__(self, other): return self[-1] != self.__get_value(other)
...
def prepare_data( data:list, origin:DataSources, **kwargs)->np.array:
    ...
    return np.fromiter(( itemgetter(*col_ids)(i) for i in data ), dtype=dtypes) if origin == DataSources.websocket else np.array( data, dtype=dtypes )
...
class Overlap:
    ...
    """    
    @staticmethod
    def sma( data:np.array, lenght:int, target:SMATarget=SMATarget.close)->TechnicalAnalysis:
        return TechnicalAnalysis(talib.SMA(data[target.name], lenght))
    ...
    """
    @staticmethod
    def bollinger(data:np.array, lenght:int=5, devup:float=2, devdown:float=2, matype: MAType=MAType.SMA)->Tuple[TechnicalAnalysis, TechnicalAnalysis]:
...
class Momentum:
    ...
    """
    @staticmethod
    def macd(data:np.array, fast:int=12, slow:int=26, signal:int=9)->Tuple[TechnicalAnalysis, TechnicalAnalysis]:
    ...
    """
    @staticmethod
    def stoch(data:np.array, fastk:int=5, slowk:int=3, slowk_type:MAType=MAType.SMA, slowd:int=3, slowd_type=MAType.SMA)->Tuple[TechnicalAnalysis, TechnicalAnalysis]:
        r = talib.STOCH(data['high'], data['low'], data['close'], fastk, slowk, slowk_type, slowd, slowd_type)
        return TechnicalAnalysis(r[0]), TechnicalAnalysis(r[1])
    ...
    def mfi(data:np.array, lenght:int=14)->TechnicalAnalysis:
        return TechnicalAnalysis(talib.MFI(data['high'], data['low'], data['close'], data['base_asset_volume'], lenght))
...
class Volume:
    ...
    def chaikin_osc(data:np.array, fast:int=3, slow:int=10)->TechnicalAnalysis:
        return TechnicalAnalysis(talib.ADOSC(data['high'], data['low'], data['close'], data['base_asset_volume'], fast, slow))
...
class Patterns:
    ...
    def morning_star(data:np.array, penetration:float=0)->TechnicalAnalysis:
        return TechnicalAnalysis(talib.CDLMORNINGSTAR(data['open'], data['high'], data['low'], data['close'], penetration))
    ...
    def evening_star(data:np.array, penetration:float=0)->TechnicalAnalysis:
        return TechnicalAnalysis(talib.CDLEVENINGSTAR(data['open'], data['high'], data['low'], data['close'], penetration))
