...
dict_test = {"key1": {"key11": {"key111": []}, "key12": {}}, "key2": {}}
...
def test_market_must_be_able_concatenate_dict_keys():
    m = Market(dict_test)
    assert isinstance(m.key1.key11, Market)


def test_market_must_return_keys():
    m = Market(dict_test)
    assert m.items == list(dict_test.keys())
    assert m.key1.items == list(dict_test["key1"].keys())


def test_market_must_check_has_items():
    m = Market(dict_test)
    assert m.has_items
...
def test_tradestrategy_must_return_market_atribute_info():
    m = Market(dict_test)
    t = TradeStrategy(m, m)
