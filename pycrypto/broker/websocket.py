...
class BinanceWebsocket(metaclass=Singleton):
    _stream: BinanceWebsocketClient = None  # type:ignore

    def __init__(self, ticker="BTCUSDT", intervals=["1s", "1m", "1h"]):
        Cache()
    ...
    def get_string_connection(self, ticker: str = None, intervals: str | list = None):
        ...
        logger.debug(Message.info.websocket_string_conn.format(string_connection))
    ...
    def start_websocket(self, *args, **kwargs) -> bool:
        try:
            match len(args):
                case 1:
                    string_connection = args[0]
                case 2:
            ...
            logger.info(Message.info.websocket_start.format(ticker, ", ".join(intervals)))
            ...
            logger.exception(e)
            raise Exception(Message.error.websocket_connection)
